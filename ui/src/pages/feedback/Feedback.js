...
const calcCellWidth = (addon = 0) => {
  ...
  const width =
    parseInt(main_config.width.slice(0, -3)) -
    cellBorderWidth * 2 +
    addon +
    'px';
  return width;
...
const BarComponent = (props) => {
  const calcRowWidth = (numCells) => {
    const cellBorderWidth = 1;
    const width =
      (parseInt(calcCellWidth().slice(0, -2)) + cellBorderWidth * 2) *
        numCells +
      cellBorderWidth * 2 +
      'px';
  ...
  return (
    <Grid data_id='bar-component' style={{}}>
      ...
      <GridRow
        style={() => ({ ...rowStyle, border: undefined })}
        data_id={'bar-component-cells-index'}
      >
  ...
  const handleAddOrUpdatePreference = (wouldGoAgain) => {
    return new Promise(async (resolve, reject) => {
      let params = {
        restaurantID: restaurantState.restaurant.id,
        email: userState.email,
        // cuisines: restaurantState.cuisines,
        review: comments,
      };

      let preference = getFoodPreferences(
        restaurantState.restaurant.preference
      );
      ...
      const preference_db = API.apiResHandling(
        res,
        messageDispatch,
        res.message
      );
      if (preference_db != null) {
        ...
        if (preference !== preference_db && wouldGoAgain) {
          ...
          mergedPreferenceArray = Object.values(
            Object.fromEntries(
              mergedPreferenceArray.map((element) => [element, element])
            )
          );
  ...
  return (
    <Grid rowGap={5} style={theme}>
      ...
      <DecisionComponent>
        ...
        <RowComponent
          theme={{ justifyContent: 'space-between', alignItems: 'center' }}
        >
